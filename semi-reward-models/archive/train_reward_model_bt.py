...
pseudo_labeled_dataset_path = os.path.join(dataset_dir,
...
pairwise_train_data_dict, pairwise_eval_data_dict, pairwise_test_data_dict = \
    load_mix_data(datasets, config, dataset_paths, total_attributes=total_attributes)

logging.info("Training Pairwise Data Size:",
             pairwise_train_data_dict["chosen_embeddings"].shape[0])
logging.info(
...
# temperature = config.get("temperature_unlabeled",1)
temperature = None
...
selected_index, pseudo_labels, pseudo_head_masks, thresholds,all_max_probs, all_outputs = trainer.generate_pseudo_labels(
    unlabel_loader, thresholds=thresholds, temperature=temperature, apply_calibration=False)

# import ipdb
# ipdb.set_trace()
# ds = load_from_disk("../dataset/400K_pairwise")['train']
# ds_selected = ds.select(selected_index)
logging.info(torch.sum(torch.stack(pseudo_head_masks,dim=0),dim=0))
...
# Evaluation
# ---------------------------
output_result_dir = os.path.join(output_dir,'best_model_result')
