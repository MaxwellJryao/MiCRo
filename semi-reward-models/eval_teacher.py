...
class ScriptArguments:
    ...
    output_result_dir: Optional[str] = field(default='./test_results/Ray2333_GRM-Llama3.2-3B-rewardmodel-ft/samples_100')
...
checkpoint_path = os.path.join(script_args.output_result_dir, 'logs','checkpoint-80')
...
# model = accelerator.prepare(trainer.model)
# dataloader = accelerator.prepare(trainer.get_eval_dataloader(test_dataset))

model = trainer.model
model.eval()
dataloader =trainer.get_eval_dataloader(test_dataset)

all_rewards_j, all_rewards_k = [], []

for step, inputs in tqdm(enumerate(dataloader), desc="Start Evaluation..."):
    inputs = trainer._prepare_inputs(inputs)
    with torch.no_grad():
        _, logits_dict = trainer.compute_loss(model, inputs, return_outputs=True)

    rj = logits_dict["rewards_j"].detach().cpu()
    rk = logits_dict["rewards_k"].detach().cpu()
    all_rewards_j.append(rj)
    all_rewards_k.append(rk)

r_chosen = torch.cat(all_rewards_j, dim=0)
r_rejected = torch.cat(all_rewards_k, dim=0)
...
ds_reward_bench = []
ds_reward_bench = load_from_disk(os.path.join(script_args.dataset_dir,'reward_bench_pairwise'))['train']
